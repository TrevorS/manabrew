...
| `meta_matchups.tsv`, `meta_baseline.jsonl`                          | Tournament decks rather than sampled ones: Mono-Green Landfall against Izzet Spellementals and the Landfall mirror, over `parity_decks/meta_*.json` (four Standard lists from one event), and the gate file of seeds 1-360 at 30 turns (720 games, all PASS). `scripts/meta-gate.sh` (`yarn parity:meta`) classifies a fresh sweep against it; any line it prints is a regression. The Java cache makes a rerun of known seeds take well under a minute, so run the whole range, not a sample. |

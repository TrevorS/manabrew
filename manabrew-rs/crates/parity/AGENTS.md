...
| `meta_matchups.tsv`                                                 | Tournament decks rather than sampled ones: Mono-Green Landfall against Izzet Spellementals and the Landfall mirror, over `parity_decks/meta_*.json` (four Standard lists from one event). Run it as a seed sweep with `--matrix --matchups meta_matchups.tsv --seeds 1,2,...`; it has no baseline file. |

...
| `meta_matchups.tsv`                                                 | Tournament decks rather than sampled ones: Mono-Green Landfall against Izzet Spellementals and the Landfall mirror, over `parity_decks/meta_*.json` (four Standard lists from one event). Run it as a seed sweep with `--matrix --matchups meta_matchups.tsv --seeds 1,2,...`; it has no baseline file. At `05168eae2` seeds 1-360 of both pairings pass at 30 turns (720 games, 717 agreeing on decisions too); a failure there is a regression. The Java cache makes a rerun of known seeds take well under a minute, so rerun the whole range, not a sample. |

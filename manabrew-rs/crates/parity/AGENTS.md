...
`gate-summary` needs no baseline and exits 1 if any game is not `PASS`; `--decisions` also lists the games that agree on state only. `explain` (`src/explain.rs`) prints four things for a game, all from the report, with no rerun. The first RNG draw the two agents disagree on, keyed by draw kind, option count, result, turn and player, and for `choose_action` also by the picked card, so the same index into a differently ordered list counts; one-option picks and Rust's `pick_many_unique` summary rows are left out, and `pick_one` reads as `pick_index`. Every snapshot field that differs at the divergence turn, with unequal card lists reduced to the names only one side has. The options that differ in the first unequal `$ACTION_SPACE` of the decision's phase. The callbacks of that phase on each side. The draw diff usually names the cause a turn or more before the state diverges: an extra or missing prompt, or an option only one engine offers. When the draws agree to the end and the state still differs, the cause is deterministic (a static, a counter, a trigger), and the snapshot diff is the place to look.

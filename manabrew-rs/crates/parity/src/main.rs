...
//! CLI entry point for `parity`.
//!
//! ```text
//! parity --deck1 <name> --deck2 <name> [--seed N] [--max-turns N]
//!              [--games N]
//!              [--java-jar <path>]
...
use parity::java_bridge::{JavaServer, JavaServerConfig};
use parity::java_cache::{self, JavaCache};
use parity::java_random::JavaRandom;
use parity::parity_compare::extract_investigation_window;
use parity::protocol::{
    FuzzReport, FuzzResult, MatchupResult, MatchupStatus, MatrixReport, ParityLogEntry,
};
use parity::report;
use parity::runner::{self, deck_search_dirs, LoadedData, RunConfig};
use parity::runtime::{JavaServerPool as ServerPool, ParityRuntime};
use parity::utils::decks::available_presets;
...
fn main() {
    ...
    if args.get(1).is_some_and(|arg| arg == "selector") {
        std::process::exit(parity::selector_audit::run_print_cli(&args[1..]));
    }
...
fn collect_unique_deck_cards(deck1: &str, deck2: &str, decks_dirs: &[&str]) -> Vec<String> {
    ...
    for deck in [deck1, deck2] {
        match parity::utils::decks::resolve_deck_spec(deck, decks_dirs) {
            Ok(spec) => {
                for (name, _) in spec {
                    cards.insert(name);
                }
...
fn run_continuous_mode(cli: &Cli) {
    use parity::infra::storage::Storage;
    use parity::scheduler::Scheduler;
...
fn run_serve_mode(cli: &Cli) {
    use parity::infra::storage::Storage;
    use parity::infra::web::{self, DashboardConfig};
    use parity::log_buffer::{BufferLayer, LogBuffer};
    use parity::scheduler::Scheduler;
    ...
    #[cfg(feature = "analyze")]
    {
        use parity::infra::analyzer::{self, AnalyzerConfig};

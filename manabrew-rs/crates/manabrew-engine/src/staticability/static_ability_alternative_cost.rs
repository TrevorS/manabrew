...
/// Keep in sync with `alternative_costs`: for its own card it collects only from `AlternativeCost`
/// statics active in that card's zone.
pub fn has_active_alternative_cost(card: &Card) -> bool {

//! Replacement logic for `Event$ Connive`.
//!
//! Mirrors Java `ReplaceConnive.java` in `forge/game/replacement/`.
...
pub fn can_replace(
    ...
    if effect.event != ReplacementType::Connive {
        return false;
    }
    let ReplacementEvent::Connive { card } = event else {
        return false;
    };

...
impl GameLoop {
    ...
    fn cast_card_spell_ability(
        ...
        let prechosen_spell_cards = if let Some(ref sc) = spell_cost {
            match self.prechoose_additional_cost_cards(game, agents, player, card_id, sc, Some(&sa))
            {
                Some(picks) => Some(picks),
                None => rollback_failed_payment!(),
            }
                ...
                if after_count < before_count {
                    ...
                    if valid_ok && counter_spec.contains("P1P1") {
                        let count = counter_spec
                            ...
                            .unwrap_or(1);
                        game.card_mut(card_id).add_etb_counter(
                            Some(player),

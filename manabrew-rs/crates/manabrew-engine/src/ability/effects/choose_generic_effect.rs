...
#[manabrew_engine_macros::spell_effect(ChooseGenericEffect)]
fn resolve(ctx: &mut EffectContext, sa: &crate::spellability::SpellAbility) {
    ...
    for chooser in choosers {
        ...
        abilities.retain(|choice_sa| {
            if !choice_sa
                .restriction
                .check_other_restrictions(ctx.game, source_id, choice_sa, player)
            {
                false
            } else if let Some(unless_cost_str) = choice_sa.ir.unless_cost.as_deref() {
        ...
        if !chosen_sas.is_empty() {
            for chosen_sa in chosen_sas {
                if sa.ir.set_chosen_mode {
                    ctx.game.card_mut(source_id).set_chosen_mode(
                        chosen_sa
                            .ir
